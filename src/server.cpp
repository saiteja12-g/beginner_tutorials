...
void add(const std::shared_ptr<MsgModify::Request> request, std::shared_ptr<MsgModify::Response> response) {
...
int main(int argc, char **argv) {
  ...
  std::shared_ptr<rclcpp::Node> node =
      rclcpp::Node::make_shared("MsgModify_server");

  rclcpp::Service<MsgModify>::SharedPtr service =
      node->create_service<MsgModify>("MsgModify", &add);
